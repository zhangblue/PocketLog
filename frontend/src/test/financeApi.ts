...
type FixtureOptions = { transactions?: Transaction[]; categories?: Category[]; accounts?: AccountLabel[]; customIcons?: string[]; pageSize?: number; fail?: Partial<Record<'create' | 'delete' | 'restore' | 'label' | 'customIcon' | 'bootstrap' | 'transactions' | 'overview' | 'analytics' | 'report' | 'revisionConflict', boolean | number>> }
...
export function createFixtureApi(options: FixtureOptions = {}): FinanceApi {
  ...
  let customIcons = [...(options.customIcons ?? [])]
  ...
  const api = {
    ...
    patchCategory: async (id: string, input: { name?: string; emoji?: string; active?: boolean }) => { const item = categories.find(row => row.id === id)!; const next = { ...item, ...input }; categories = categories.map(row => row.id === id ? next : row); revision++; return { data: dtoCategory(next), dataRevision: revision } },
